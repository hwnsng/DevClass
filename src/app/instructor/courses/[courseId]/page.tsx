...
export default function StudentsPage() {
  ...
  if (error || !course) return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
      ...
      <h2>강의 정보 없음</h2>
      <Link href="/instructor" style={{ color: "#20B486", fontWeight: 600, textDecoration: "none" }}>← 대시보드로</Link>
    </div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f8faf9" }}>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "48px 24px" }}>
        <Link href="/instructor" style={{ color: "#20B486", textDecoration: "none", fontSize: 13, fontWeight: 600 }}>
          ← 대시보드로
        </Link>
        <h1 style={{ fontSize: 26, fontWeight: 800, color: "#1a1a2e", margin: "16px 0 6px" }}>
          {course.title}
        </h1>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16, marginBottom: 32 }}>
          {[
            ...
            <div key={s.label} style={{ background: "#fff", borderRadius: 14, padding: "20px 24px", boxShadow: "0 2px 12px rgba(0,0,0,0.06)", display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <div>
                <div style={{ fontSize: 22, fontWeight: 800, color: "#20B486" }}>{s.value}</div>
        ...
        {students.length === 0 ? (
          ...
          <div style={{ background: "#fff", borderRadius: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.06)", overflow: "hidden" }}>
            <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", padding: "16px 24px", background: "#f8faf9", borderBottom: "1px solid #eee", fontSize: 13, fontWeight: 700, color: "#888" }}>
              <span>수강생</span>
              <span>진도율</span>
              <span>수강 시작일</span>
            </div>
            {students.map((s: any) => (
              <div key={s.userId} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", padding: "18px 24px", borderBottom: "1px solid #f5f5f5", alignItems: "center" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 36, height: 36, background: "linear-gradient(135deg,#20B486,#17926d)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 14, fontWeight: 700 }}>
                    {s.name?.[0] ?? "?"}
                  </div>
                  <span style={{ fontSize: 15, fontWeight: 600, color: "#1a1a2e" }}>{s.name}</span>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ flex: 1, height: 6, background: "#f0f0f0", borderRadius: 3, overflow: "hidden" }}>
                    <div style={{ height: "100%", width: `${s.progressPercent}%`, background: "linear-gradient(90deg,#20B486,#17926d)", borderRadius: 3 }} />
                  </div>
